    ...
    try:
        with st.spinner("Memproses fail..."):
            ...
            if not original_text.strip():
                st.warning(
                    "Tiada teks dapat dibaca. Jika fail ini ialah gambar, "
                    "pastikan gambar jelas dan Tesseract OCR sudah dipasang."
                )
            else:
        ...
        st.code(str(e))
        st.info(
            "Untuk gambar JPG/PNG, ralat paling biasa ialah Tesseract OCR belum dipasang "
            "atau path tesseract.exe tidak betul."
        )
